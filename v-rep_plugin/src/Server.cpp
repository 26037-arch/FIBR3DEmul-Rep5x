...
SessionEcho::SessionEcho(boost::asio::io_context * io_service) : socket_(*io_service)
{
...
SessionSend::SessionSend(boost::asio::io_context * io_service) : socket_(*io_service)
{
...
Server::Server(boost::asio::io_context* io_service, short port1, short port2) : io_service_(io_service),
...
void CommunicationManager::LaunchServer()
{
	try
	{
		...
		// Launch the Asio context in another thread. io_service was removed in
		// current Boost releases; io_context is its source-compatible successor.
		bt = std::make_unique<boost::thread>(boost::bind(&boost::asio::io_context::run, &ios));
	}
