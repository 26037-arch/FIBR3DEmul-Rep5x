...
bool Listener::Deserialize(const std::string& JSON_msg)
{
	try
	{
		ptree request;
		...
		if (coord1.size() != 5 || coord2.size() != 5 || vel.size() != 3 || acc.size() != 3)
			throw std::runtime_error("JSON schema mismatch: pos1/pos2 must contain 5 values and vel/acc must contain 3 values");
		int i = 0, j = 0, k = 0, l = 0;
		for (ptree::value_type &el : coord1)
		{
			m_pack.vec1[i] = el.second.get<double>("");
			if (!std::isfinite(m_pack.vec1[i])) throw std::runtime_error("NaN/Infinity in pos1");
			i++;
		}
		for (ptree::value_type &el : coord2)
		{
			m_pack.vec2[j] = el.second.get<double>("");
			if (!std::isfinite(m_pack.vec2[j])) throw std::runtime_error("NaN/Infinity in pos2");
			j++;
		}
		for (ptree::value_type &el : vel)
		{
			m_pack.vel[k] = el.second.get<double>("");
			if (!std::isfinite(m_pack.vel[k])) throw std::runtime_error("NaN/Infinity in vel");
			k++;
		}
		for (ptree::value_type &el : acc)
		{
			m_pack.acc[l] = el.second.get<double>("");
			if (!std::isfinite(m_pack.acc[l])) throw std::runtime_error("NaN/Infinity in acc");
			l++;
		}
		return true;
	}
	...
	{
		std::string emsg = std::string("Parser/plugin JSON command rejected: ") + error.what();
		v_repExtPQBarMsgCommunication(emsg);
		v_repExtPQBoxMsgCommunication(emsg);
		return false;
	}
}
...
void Listener::PushAction()
{
	...
	if(m_pack.keycode < 2000) //Motion commands
	{
		...
		case Dwell:
			...
			break;
		default:
			v_repExtPQBoxMsgCommunication("Unsupported motion keycode from parser at G-code line " + std::to_string(m_pack.line));
			return;
		}
