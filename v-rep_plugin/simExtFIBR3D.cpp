...
unique_ptr<Rep5xMotionAdapter> ptr_rep5x_adapter;
...
void LUA_INIT_FIBR3D_CONTROL_CALLBACK(SScriptCallBack* p)
{
	...
	if (inArguments.getSize()>=2 && inArguments.isString(0) && inArguments.isNumber(1)) 
	{
		...
		const bool rep5x_mode = (pname == "Rep5x_Ender3V3SE" && pdofs == 5);
		ptr_extruder = make_unique<Extruder>(rep5x_mode ? "Rep5x_NozzleTip" : "Extruder", "DrawBoard");
		ptr_cm = make_unique<CommunicationManager>();
		if (rep5x_mode)
		{
			try
			{
				Rep5xMachineProfile profile;
				if (inArguments.getSize() >= 3 && inArguments.isString(2))
					profile = Rep5xMachineProfile::LoadJson(inArguments.getString(2).c_str());
				ptr_rep5x_adapter = make_unique<Rep5xMotionAdapter>(profile);
			}
			catch (const std::exception& error)
			{
				printf("simExtFIBR3D Rep5x profile error: %s\n", error.what());
				ptr_rep5x_adapter.reset();
				plugin_loaded = false;
				return;
			}
...
void LUA_CLEANUP_FIBR3D_CONTROL_CALLBACK(SScriptCallBack* p)
{
	...
	if (inArguments.getSize() == 0)
	{
		if (plugin_loaded) {
			...
			ptr_rep5x_adapter.reset(nullptr);
...
void v_repExtPQSetJointTrajectory(std::vector<std::vector<double> > & dpos, const std::vector< std::pair<bool, int> > &extrude, const std::vector<int> &line)
{
	if (ptr_printer->ready)
	{
		if (dpos.size() != extrude.size() || dpos.size() != line.size())
		{
			v_repExtPQBoxMsgCommunication("Trajectory queue producer mismatch: position/extrusion/line counts differ.");
			return;
		}
		std::vector<std::vector<double> > scene_positions;
		try
		{
			scene_positions.reserve(dpos.size());
			for (size_t i = 0; i < dpos.size(); ++i)
				scene_positions.push_back(ptr_rep5x_adapter ? ptr_rep5x_adapter->ToSceneJoints(dpos[i]) : dpos[i]);
		}
		catch (const std::exception& error)
		{
			v_repExtPQBoxMsgCommunication(std::string("Rep5x trajectory rejected: ") + error.what());
			return;
		}
		// Push target positions and extrude property
		for (size_t i = 0; i < scene_positions.size(); ++i)
		{
			ptr_printer->target_jpos.push_front(scene_positions[i]);
			//push paint and type
...
namespace _internal_ {
	...
	void v_repExtGetHandles(Printer * printer, Extruder * extruder)
	{
		...
		if (phandle != -1)
		{
			...
			if (ret != nullptr)
			{
				...
				if (ptr_rep5x_adapter)
				{
					for (size_t i = 0; i < 5; ++i)
						printer->joint_handles[i] = simGetObjectHandle(ptr_rep5x_adapter->profile().axes[i].joint_name.c_str());
				}
				else
				{
					// Original FIBR3D scene contract: X,Y,Z,B,C.
				...
				for (size_t i = 0; i < printer->joint_handles.size(); ++i)
				{
					if (printer->joint_handles[i] == -1)
					{
						msgBox_scq.push_back("Missing required printer joint. Check the Rep5x scene/profile object names.");
						return;
					}
					unique_handles.insert(printer->joint_handles[i]);
				}
				if (unique_handles.size() != printer->joint_handles.size())
				{
					msgBox_scq.push_back("Duplicate required joint handles detected. Scene object aliases must be unique.");
					return;
				}
				printer->ready = true;
			}
		...
		const int extrude_type)
	{
		...
		{
			msgBox_scq.push_back("Number of joint handles does not match the trajectory sample schema.");
			return false;
		}
		...
		//Set extruder
		// Phase 1 animates Rep5x motion only.  The legacy filament writer assumes
		// the original Cartesian joint order, so deliberately do not emit a
		// misleading extrusion mesh for the Rep5x profile.
		if (extrude && !ptr_rep5x_adapter)
		{
